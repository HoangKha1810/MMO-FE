...
interface AdminAutoMxhApiDocsPageProps {
  baseUrl: string;
  sections: AutoMxhDocsCatalogSection[];
...
function PriceRow({ service }: { service: AutoMxhApiDocsService }) {
  return (
    <tr className="align-top">
      ...
      <td className="px-4 py-4">
        ...
        <div className="mt-1 text-[11px] font-semibold text-slate-500 dark:text-slate-400">
          {service.api_service_id || 'Chưa map'}
        </div>
      </td>
      <td className="px-4 py-4 text-right">
        ...
        <div className="mt-1 text-[11px] font-semibold text-slate-500 dark:text-slate-400">
          Tổng {formatMoney(service.total_to_pay)} đ
        </div>
      </td>
    </tr>
  );
...
export function AdminAutoMxhApiDocsPage({
  baseUrl,
  sections,
  runtimeMeta,
  loadedAt,
  ...
  const endpointNav = useMemo(
    () => [
      ...
      { id: 'catalog', label: 'Catalog', icon: <Layers className="h-4 w-4" /> },
      { id: 'category', label: 'Category', icon: <Layers className="h-4 w-4" /> },
      { id: 'quote', label: 'Quote', icon: <ShoppingCart className="h-4 w-4" /> },
  ...
  const filteredServices = useMemo(() => {
    ...
    const sorted = [...docs.services].sort((a, b) => a.category_name.localeCompare(b.category_name) || a.product_id - b.product_id || a.id - b.id);
    ...
    <div className="space-y-6">
        ...
        stats={[
          {
            ...
            hint: 'Domain web đang phục vụ API AutoMXH',
            tone: 'blue',
          },
          {
            label: 'Services',
            value: String(docs.summary.totalServices),
            hint: `${docs.summary.totalProducts} product · ${docs.summary.totalCategories} category`,
            tone: 'emerald',
          },
          {
            label: 'VAT',
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          ...
          value="/api/external/automxh/services"
          hint="Cần x-api-key"
          tone="blue"
        ...
        />
        <MetricCard
          label="Endpoint tạo đơn"
          value="/api/external/automxh/order"
          hint="Trừ ví chính users.balance"
          tone="emerald"
        ...
        <MetricCard
          label="VAT"
          value={`${runtimeMeta.vatPercent ?? 0}%`}
          hint="Tính sau giá gói"
          tone="amber"
      ...
      <SectionPanel className="space-y-5">
        <SectionHeader
          eyebrow="Live Prices"
          title="Giá Web Hiện Tại Từ AutoMXH"
          description="Bảng này dùng data AutoMXH active trong DB. Mặc định hiện 40 gói đầu; tìm kiếm sẽ lọc tối đa 60 dòng."
          actions={
            <div className="flex flex-wrap gap-2">
              <input
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Tìm variant, product, category, provider..."
            ...
            <table className="w-full min-w-[1080px] text-left">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                {filteredServices.length > 0 ? (
                  filteredServices.map((service) => <PriceRow key={`${service.product_id}-${service.variant_id}`} service={service} />)
                ) : (
                  <tr>
