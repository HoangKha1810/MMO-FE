...
export default async function SellerDashboardPage() {
  ...
  const activeItems = dashboard.items.filter((item) => String(item.status || '') === 'selling').length;
      ...
      <div className="space-y-6">
        <section className="relative overflow-hidden rounded-[2.25rem] border border-slate-200 bg-[#f7f3ea] p-7 dark:border-white/10 dark:bg-[#101520]">
          ...
          <div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
            <div>
              <div className="inline-flex rounded-full bg-emerald-500/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.25em] text-emerald-500">Kênh người đăng</div>
              <h1 className="mt-4 text-4xl font-black uppercase tracking-[-0.06em] text-slate-950 dark:text-white md:text-5xl">Dashboard người đăng</h1>
              <p className="mt-4 max-w-2xl text-sm font-semibold leading-7 text-slate-600 dark:text-slate-300">
                Quản lý bài trao đổi game, đơn trao đổi và yêu cầu rút tiền trong một dashboard riêng.
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Link href="/user/game-market/sell" className="rounded-xl bg-brand-blue px-4 py-2 text-xs font-black uppercase text-white">Đăng bài trao đổi</Link>
              <Link href="/user/seller/orders" className="rounded-xl bg-slate-950 px-4 py-2 text-xs font-black uppercase text-white dark:bg-white dark:text-slate-950">Đơn trao đổi</Link>
        ...
        <section className="grid gap-4 md:grid-cols-5">
          {[
            { label: 'Bài trao đổi', value: dashboard.items.length, icon: Box },
            { label: 'Đơn trao đổi', value: dashboard.orders.length, icon: ReceiptText },
            { label: 'Đang hiển thị', value: activeItems, icon: Box },
            { label: 'Đã trao đổi', value: dashboard.completedSales, icon: PackageCheck },
        ...
        <section className="grid gap-6 xl:grid-cols-[1fr_420px]">
          <div className="rounded-[2rem] border border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-slate-900">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-950 dark:text-white">Bài trao đổi đang hiển thị</h2>
            <div className="mt-4 space-y-3">
              {dashboard.items.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-sm font-bold text-slate-400 dark:border-white/10">Chưa có bài trao đổi.</div>
          ...
          <aside className="rounded-[2rem] border border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-slate-900">
            <h2 className="text-sm font-black uppercase tracking-widest text-slate-950 dark:text-white">Đơn trao đổi mới</h2>
            <div className="mt-4 space-y-3">
              {dashboard.orders.slice(0, 10).map((order) => (
                <div key={String(order.id)} className="rounded-2xl bg-slate-50 p-4 dark:bg-white/[0.03]">
                  <div className="text-sm font-black text-slate-950 dark:text-white">#{String(order.id)} {String(order.item_title || 'Bài trao đổi')}</div>
